...
Note that while better-auth docs specify to use `auth` on the server, we have made `authClient` on isomorphic. ALWAYS use the authClient function in the `src/routes` folders and only `auth` in the `src/server` folder. Since authClient is isomorphic you can simply use it in loaders and with tanstack start.
